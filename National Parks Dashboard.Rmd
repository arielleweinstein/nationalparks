...
generate_llm_explanation <- function(states, activities, amenities) {
  ...
  prompt <- paste0(
    ...
    "Write a short, human-readable explanation in 2–3 sentences, including the information of state. Note that use parks instead of park and write your idea about the state user selected."
  )
...
generate_llm_explanation <- function(states, activities, amenities) {
  ...
  # 打印原始返回，方便调试
  txt <- content(res, "text", encoding = "UTF-8")
  result <- fromJSON(txt, simplifyVector = FALSE)  # ❗关键：禁用简化

  # 从嵌套列表中提取 message$content
...
generate_llm_explanation(
  c("California", "Texas"),
  c("Hiking", "Camping", "Wildlife Viewing"),
  c("Restrooms", "Picnic Areas", "Visitor Centers")
)
```